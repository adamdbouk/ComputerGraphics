...
void Application::init(void)
{
	...
	//load the texture
	texture = new Image();
	texture_normals = new Image();
	texture->loadTGA("color.tga");
	texture_normals->loadTGA("lee_normal.tga");
	zBuffer.fill(10000);	// Emplenem el zbuffer amb valors molt grans perquè així s'actualitzaran segur

	light = new Light();
	material = new Material();
	//ambient_light = Vector3(material->ambient.x * light->ambient_color.x, material->ambient.y * light->ambient_color.y, material->ambient.z * light->ambient_color.z);
}
...
void Application::render(Image& framebuffer)
{
	...
	for (unsigned int i = 0; i < mesh->vertices.size(); i += 3)
	{
		...
		Vector3 n0 = mesh->normals[i];
		Vector3 n1 = mesh->normals[i + 1]; 
		Vector3 n2 = mesh->normals[i + 2]; 
		...
		framebuffer.drawTriangle(projected_v0, projected_v1, projected_v2, uv0, uv1, uv2, &zBuffer, texture, light, material, v0, v1, v2, ambient_color, camera, texture_normals);
	}
}
...
void Application::onKeyDown( SDL_KeyboardEvent event )
{
	...
	case SDL_SCANCODE_T:
		this->mode = 3; // Canviem la variable fill per alternar si volem veure la textura o només la xarxa de triangles
		break;

	case SDL_SCANCODE_M:
		this->mode = 1; // Canviem la variable fill per alternar si volem veure la textura o només la xarxa de triangles
		break;

	case SDL_SCANCODE_I:
		this->mode = 2; // Canviem la variable fill per alternar si volem veure la textura o només la xarxa de triangles
		break;
