...
void Image::drawTriangle(Vector3 p1, Vector3 p2, Vector3 p3, Vector2 t1, Vector2 t2, Vector2 t3, Vector3 lv1, Vector3 lv2, Vector3 lv3, Color col, FloatImage* zbuffer, Image* texture, int mode, Light* light, Material* material)
//Donat tres punt i un boleà aquesta funció ens dibuixa un triangle.
{
	...
	this->bresenhamTriangle(x1, y1, x2, y2, col, table);
	this->bresenhamTriangle(x1, y1, x3, y3, col, table);
	this->bresenhamTriangle(x2, y2, x3, y3, col, table);
	...
	for (int j = 0; j < this->height; j++) {
		if (table[j].maxx > -10000 && table[j].minx < 10000) {
			for (int i = table[j].minx; i <= table[j].maxx; i++) {
				...
				//Calculem la iluminació ponderada al pixel en el que estem
				float lx = lv1.x * u + lv2.x * v + lv3.x * w;
				float ly = lv1.y * u + lv2.y * v + lv3.y * w;
				float lz = lv1.z * u + lv2.z * v + lv3.z * w;
				l = Vector3(clamp(lx, 0, 1), clamp(ly, 0, 1), clamp(lz, 0, 1));
					...
					if (newY < texture->height && newY >= 0) {
						if (i < this->width && i >= 0) {
							if (zbuffer->getPixel(i, j) >= z) { // Mirem si el pixel que volem pintar esta més aprop que el que hi ha posat al zbuffer
								...
								{
								case 1:
									if (i == table[j].minx || i == table[j].maxx) {
										paint_point(i, j, col);
									}
									break;
								...
								case 3:
									...
									float y = t.g / 255.0;
									float z = t.b / 255.0;
									float red = clamp((material->ambient.x * x) + l.x, 0, 1);
									float green = clamp((material->ambient.y * y) + l.y, 0, 1);
									float blue = clamp((material->ambient.z * z) + l.z, 0, 1);
									t = Color(floor(red * 255), floor(green * 255), floor(blue * 255));
									paint_point(i, j, t);
									break;
								}
							}
