...
//Vector3 ambient_color = Vector3(0.3f, 0.3f, 0.3f);
...
void Application::init(void)
{
	...
	// Calculem ambient light aqui per anar més rapids
}
...
void Application::render(Image& framebuffer)
{
	...
	for (unsigned int i = 0; i < mesh->vertices.size(); i += 3)
	{
		...
		Vector3 n0 = mesh->normals[i];	   //extract normal from mesh
		Vector3 n1 = mesh->normals[i + 1]; //extract normal from mesh
		Vector3 n2 = mesh->normals[i + 2]; //extract normal from mesh
		...
		Vector3 lv1 = camera->calcLight(v0, n0, light, material, ambient_light); //Calculem la iluminacio en cada vertex del triangle
		Vector3 lv2 = camera->calcLight(v1, n1, light, material, ambient_light);
		Vector3 lv3 = camera->calcLight(v2, n2, light, material, ambient_light);
		...
		framebuffer.drawTriangle(projected_v0, projected_v1, projected_v2, uv0, uv1, uv2, lv1, lv2, lv3, Color::BLACK, &zBuffer, texture, this->mode, light, material);
	}
}
